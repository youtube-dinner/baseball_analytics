...
ANALYTICS="$ROOT/outputs/minor_league_pitcher_stars.py"
DASHBOARD="$ROOT/work/build_minor_league_hitter_dashboard.py"
EXPORT_DIR="$ROOT/outputs/minor_league_pitcher_stars/fangraphs_exports"
OUTPUT_BASE="$ROOT/outputs/minor_league_pitcher_stars"
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/minor_league_pitcher_${YEAR}_supervised_refresh_$(date +%Y%m%d_%H%M%S).log"
...
{
  echo "Started supervised FanGraphs minor-league pitcher refresh at $(date)"
  echo "Year: $YEAR"
  ...
  echo "Exporting FanGraphs minor-league pitcher reports"
  "$NODE" "$EXPORTER" "${export_args[@]}"

  echo
  echo "Building minor-league pitcher analytics"
  "$PYTHON" "$ANALYTICS" \
    --year "$YEAR" \
    --leagues "$LEAGUES" \
  ...
  echo
  echo "Building combined minor-league dashboard"
  "$PYTHON" "$DASHBOARD"

  echo
  echo "Finished supervised FanGraphs minor-league pitcher refresh at $(date)"
